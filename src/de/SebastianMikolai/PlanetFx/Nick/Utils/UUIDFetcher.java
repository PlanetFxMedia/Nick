...
public class UUIDFetcher {
	...
	public static UUID getUUID(String name) {
		UUID id = null;
		try {
			id = getUUIDAt(name, System.currentTimeMillis());
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return id;
	...
	public static UUID getUUIDAt(String name, long timestamp) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(
					String.format(UUID_URL, new Object[] { name, Long.valueOf(timestamp / 1000L) })).openConnection();
	...
	public static String getName(UUID uuid) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(
					String.format(NAME_URL, new Object[] { UUIDTypeAdapter.fromUUID(uuid) })).openConnection();
			connection.setReadTimeout(5000);
